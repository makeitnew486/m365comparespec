...
| 탭 | 내용 |
|---|---|
| **홈** | Hero · **Advisory bar (E7 출시 + 7월 가격 공지)** · 왜 구독형 (3 pillar) · 둘러보기 4링크 — 다이어트되어 가벼움 |
| **플랜** | E3·E5·**E7 🆕**·Teams 제외·Teams 단독·Copilot·**Agent 365 🆕** 카드 (각 카드 list 4줄로 그룹화 압축) + 고객 유형별 추천 |
| **기능** | **압축 매트릭스 11행 7열** (E7 컬럼 포함, sticky thead) — 그룹화된 한 줄에 sub 디테일 + 33개 앱 카탈로그 (클릭→상세 모달) |
| **라이선스** | 영구형 카탈로그 8종 + 영구형 vs 구독형 비교 + 9가지 전환 이유 + 6가지 반박 처리 + **프로페셔널 서비스 로드맵** + **왜 FBKR 5장** (홈에서 이전) |
| **영업 자료** | **🎯 고객 제안 5단계** (홈에서 이전) + 15개 영업 팁 (계약·반론·제품·프로세스 필터) + 27개 용어 사전 (검색·카테고리 필터) |
