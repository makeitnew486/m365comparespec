...
| 항목 | 금지 사유 | 확인일 |
|---|---|---|
| **₩ 가격 표기** (E3·E5·E7·Copilot·Agent 365 등) | FBKR 영업 견적이 공시가와 다를 수 있어 혼동 방지. 한국 가격 공시되어 있어도 표기 X | 2026-05-11 / 재확인 2026-06-07 |
...
## 1.6. 🎯 명료성 우선 원칙 (2026-06-07 신설)

영업 입장에서 **한눈에 들어오는 것 최우선**. 새 콘텐츠·UI 추가 시 항상 자문할 것:

- "한 페이지에 한 메시지" — 두세 메시지 섞이면 분리
- "정보량 < 명료성" — 데이터는 sub 디테일(작은 글씨)로 보존하되 그룹 라벨이 우선
- "카드 4줄이면 충분" — 6~7개 ✓ 보다 4개 굵은 카테고리 묶음
- "탭 narrative 자연스럽게" — 왜 구독형(인식 전환)/플랜(상품)/기능(스펙)/라이선스(전환)/영업자료(실무) — 콘텐츠는 그 narrative 따라 배치

**적용 사례:**
- 홈 다이어트 17 카드 → 7 카드 (DD-012)
- 매트릭스 29행 → 11행 그룹화 (DD-012)
- Plan card list 6~7 ✓ → 3~4 그룹 (DD-013)
- 노티스 박스 2개 → 1박스 2행 compact (DD-013)
...
### DD-012 — 홈 다이어트 + 매트릭스 압축 (2026-06-07)
**왜:** 사용자가 "스타일 지저분 / 영업 입장 명료성 부족 / 나열 깔끔하게" 라고 종합 피드백. 정보량은 많은데 한눈에 안 들어오는 게 핵심 문제.
**원칙:** 명료성 우선. 한 페이지 한 메시지. 데이터는 sub 디테일로 보존.
**홈 다이어트:** 17 카드 → 7 카드.
- 제거: 고객 제안 5단계 (.journey-flow) / 프로페셔널 서비스 로드맵 (.roadmap-flow) / 왜 FBKR (.reason-grid 5장)
- 남김: Hero · Advisory bar · 왜 구독형 (3 pillar) · 둘러보기 · footer
- 이동: 5단계 → 영업 자료 탭 sub-section / 로드맵 + 왜 FBKR → 라이선스 탭 끝
- 이동 근거: 영업 narrative와 탭 주제 매칭 — "고객에게 어떻게?" (영업 자료) / "구독 후 무엇?" (라이선스 마무리)
**매트릭스 압축:** 29행 → 11행.
- 관련 기능을 카테고리 단위 1행으로 그룹화, 하위 컴포넌트는 `<div class="sub">`에 11.5px muted로 표시
- 예: 보안 7행 → "기본 보안 통합 (Entra ID · Intune · Defender · Purview · Windows 11 · Hello · Shadow IT)" 1행
- 첫 컬럼 width 260 → 320px (그룹 라벨 + sub 수용)

### DD-013 — Compact advisory bar + plan card 그룹화 (2026-06-07, 이전 단계)
**왜:** 사용자 1차 피드백 ("스타일 지저분") 대응으로 노티스 박스 2개와 plan card list가 너무 빽빽함.
**적용:**
- 홈 노티스 2개 큰 박스 → 1박스 2행 compact advisory bar (.advisory + .advisory-row)
- Plan card 본문 list 6~7 ✓ → 3~4 그룹화 (✓ **카테고리 굵게** — 컴포넌트 나열)
- typography hierarchy 강화 (h1 32→36, h2 22→24, h3 20→22, padding ↑)
**평가:** 부분 개선이었음. 사용자가 "근본 문제는 안 풀렸다" 추가 피드백 → DD-012로 이어짐.
